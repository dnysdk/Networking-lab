...
public class Simulation {
	...
	public static void main(String[] args) {
		...
		int totalsources = in.nextInt();
		System.out.println("how much time ");
		int time = in.nextInt();
		int timedivision = 10;
		System.out.println("enter packet size of each packet");
		int packetsize = in.nextInt();
		System.out.println("enter packet sending rate(packets per unit time) for source");
		int psr = in.nextInt();// constant for all sources 
		...
		for(int nos=1;nos<=totalsources;nos++){
			...
			case 1:
				...
				System.out.println("average sending delay if number of sources  are "+nos+" is = "+averagedelay);
				break;
